...
onMounted(async () => {
  ...
  local.value = JSON.parse(localStorage.getItem('jobs')) || assignments.value;
...
<template>
  ...
  <div class="max-w-225 m-auto p-5">
    <h1 class="mb-6 text-slate-100 text-3xl font-bold">
      Opdrachten - Digital Project Managers
    </h1>
    ...
    <div class="flex gap-3 mb-6 flex-wrap">
      ...
      <input
          ...
          type="text"
          v-model="zoekbalkInput"
          placeholder="Zoek op titel of beschrijving..."
      >

      <select
          id="select"
          class="p-[10px_14px] border border-slate-700 rounded-md bg-slate-900 text-base text-slate-300"
    ...
    <div class="box-border m-0 p-0" v-else-if="loading">
          ...
          <NuxtLink :to="`/opdrachten/${job.title}`">
            ...
            <div class="grid grid-cols-2">
              ...
              <div>
                <h2
                    id="jobTitle"
                    class="text-lg font-bold mb-2 text-slate-300"
                >
                  {{ job.title }}
                </h2>
              </div>
            ...
            <div
                class="text-slate-500 text-base mb-2"
            >
              {{ job.client.companyName }}
              <span>&#183;</span>

              {{ job.location || 'Geen Locatie' }}
              <span>&#183;</span>

              {{ job.hoursPerWeek }} uur/week
              <span>&#183;</span>
            ...
            <div
                id="jobDescription"
                class="text-slate-400 text-base leading-normal"
            >
              {{ job.description }}
            </div>
      ...
      <div
          id="geenResultaten"
          v-else
          class="flex justify-center p-10 text-slate-700"
      >
        <h1>{{ emptyReturn }}</h1>
      </div>
